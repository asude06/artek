...
blue_points = [deque(maxlen=512)] # mavi noktaları burda saklıycaz
...
paintWindow = np.zeros((471,636,3))+255 # beyaz çizim ekranı
...
while 1:
    ret,frame = cap.read()
    frame = cv2.flip(frame,1) # y eksenine göre simetriğini alıyor
    ...
    if len(contours) > 0:
        ...
        ((x,y),radius) = cv2.minEnclosingCircle(max_contours) # sınırlayıcı bir çember
        cv2.circle(frame,(int(x),int(y)),int(radius),(255,255,0),3)

        M = cv2.moments(max_contours) # merkez noktaları bulucaz
        center = (int(M["m10"]/M["m00"]),int(M["m01"]/M["m00"]))

        if center[1] <= 65: #y değeri
            if 40 <= center[0] <= 140:
                blue_points = [deque(maxlen=512)]  # mavi noktaları burda saklıycaz
                green_points = [deque(maxlen=512)]
                red_points = [deque(maxlen=512)]
