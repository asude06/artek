#webcamdan şekillerin görörüntüünü alıp belirliycez ve adını yazıcaz
#kızak oluşturuyoruz
...
cap = cv2.VideoCapture(0) #0 dediğimizde webcamden alıyor
...
while 1:
    ...
    lh = cv2.getTrackbarPos("Lower-Hue","Settings") # hue renk tonu
    ls = cv2.getTrackbarPos("Lower-Saturation", "Settings") # saturation doygunluk canlılık
    ...
    #beyaz nesnelerin üzerinde oluşan siyah noktaları silmak için kernel kullanıyoruz
    kernel = np.ones((5,5),np.uint8)
    mask = cv2.erode(mask,kernel) # erozyona uğratıcaz

    contours,_ = cv2.findContours(mask,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)

    for cnt in contours: # cnt center yani merkez demek
